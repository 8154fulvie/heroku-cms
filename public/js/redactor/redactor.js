...
(function($){
	...
	function Construct(el, options) 
	{
		this.opts = $.extend({	
			...
			imageGetJson: '/tests/images.json', // url or false
			...
			containers: new Array("p","pre","blockquote","h1","h2","h3","h4","h5","h6")				
	...
	Construct.prototype = {
		...
		shortcuts: function()
		{
			$(this.doc).keydown(function(e)
			{
				...
				// Ctrl + Shift + z	
				else if (key == 90 && e.ctrlKey && e.shiftKey)
				{
					if (e.preventDefault) e.preventDefault();
		...
		{		
			if (this.opts.visual && this.doc)
			{
    			try
	    		{
	    			...
	    			if (cmd == 'formatblock' && $.browser.msie) this.doc.execCommand(cmd, false, '<' +param + '>');	
	    			else   			
					{											
		...
		clean: function(marker)
		{
			...
			if (this.opts.removeClasses && this.opts.removeStyles)
			{
				html = html.replace(/ style="([\w\W]*?)"/gi, ''); 			
				html = html.replace(/ class="([\w\W]*?)"/gi, '');
											
			}
			else if (this.opts.removeStyles === false && this.opts.removeClasses)
			{
				html = html.replace(/ class="([\w\W]*?)"/gi, '');	
				html = this._cleanStyles(html);		
			}
			else if (this.opts.removeStyles && this.opts.removeClasses === false)	
			{
				html = html.replace(/ style="([\w\W]*?)"/gi, '');
				html = this._cleanClasses(html);							
			}		
			else
			{
				html = this._cleanStyles(html);
		...
		buildToolbar: function()
		{
			...
			$.each(RTOOLBAR, function(key,s)
			{
				...
				if (key == 'backcolor' || key == 'fontcolor' || typeof(s.dropdown) != 'undefined')
				{
					...
					this.hdlShowerDropDown = function(e) { this.showerDropDown(e, dropdown, key) }.bind2(this);   	
		...
		{		
			...
			else
			{
				...
				this.showingDropDown(e, dropdown, key);
			}						
		},
		showingDropDown: function(e, dropdown, key)
		{
		...
		showerDropDown: function(e, dropdown, key)
		{
			if (this.showedDropDown) this.showingDropDown(e, dropdown, key);
		},
		...
		*/				
        ...
        imageEdit: function(e)
        {
            var handler = function()
            {
                ...
                var floating = $el.css('float');
                if (floating == 'none') floating = 0;
                
                $('#redactor_form_image_align').val(floating);
        ...
        showImage: function()
        {
            ...
            var handler = function()
            {
				...
				else
				{
					...
					$('#redactor_tabs a').eq(0).addClass('redactor_tabs_act');
					$('#redactor_tabs2').hide();
					$('#redactor_tabs1').show();					
				}            
		...
		insertLink: function()
		{
			...
			if (a)
			{
				...
				else
				{
		            ...
		            else redactorActive.execCommand('inserthtml', a);
				}
			}
		...
		toggle: function()
		{
			...
			else
			{
				...
				this.setCodeEditor(this.getCodeTextarea());	
